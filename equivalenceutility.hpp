...
class EquivalenceUtility
{
    ...
    /* Collecting different possible x's in A in a Rule like A=FORALL(x)A=THEREEXISTS(x)A where x does not occur free in A and x is not matched */
    LogicSet *candidateBoundVariableSet;

    /* An identifier for potential candidate for candidateBoundVariableSet, usually collected and matched later on */
